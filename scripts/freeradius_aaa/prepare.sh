...
if [[ ! -d "$FR_DIR/raddb" ]]; then
  echo "ERROR: FreeRADIUS submodule not found at $FR_DIR"
...
rm -rf "$OUT_DIR"
mkdir -p "$OUT_DIR"
cp -a "$FR_DIR/raddb" "$RDB_DIR"
cp -a "$FR_DIR/share/dictionary" "$DICT_DIR"
...
if [[ -f "$RDB_DIR/sites-enabled/default" ]]; then
  ...
  perl -0777 -i -pe 's/\n\s*eap\s*\{\n\s*ok\s*=\s*return\n\s*updated\s*=\s*return\n\s*\}/\n\t# eap block disabled for EDHOC AAA benchmark\n\t# eap {\n\t# \tok = return\n\t# \tupdated = return\n\t# }/s' "$RDB_DIR/sites-enabled/default"
...
if [[ -f "$RDB_DIR/sites-enabled/inner-tunnel" ]]; then
  perl -0777 -i -pe 's/\n\s*eap\s*\{\n\s*ok\s*=\s*return\n\s*updated\s*=\s*return\n\s*\}/\n\t# eap block disabled for EDHOC AAA benchmark\n\t# eap {\n\t# \tok = return\n\t# \tupdated = return\n\t# }/s' "$RDB_DIR/sites-enabled/inner-tunnel"
  perl -0777 -i -pe 's/\n\s*eap\s*\{\n\s*ok\s*=\s*return\n\s*\}/\n\t# eap block disabled for EDHOC AAA benchmark\n\t# eap {\n\t# \tok = return\n\t# }/s' "$RDB_DIR/sites-enabled/inner-tunnel"
...
edhoc_Type0_classic Password.Cleartext := "edhoc-pass"
edhoc_Type0_PQ Password.Cleartext := "edhoc-pass"
edhoc_Type3_Classic Password.Cleartext := "edhoc-pass"
edhoc_Type3_PQ Password.Cleartext := "edhoc-pass"
edhoc_Type3_Hybrid Password.Cleartext := "edhoc-pass"
EOF
fi
